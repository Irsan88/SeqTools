...
#awk -F'[:\t]' '{for(i=1;i<=NF;i++)if(i<=7||i>=13)printf$i"\t";printf$8"\t";print""}' \
#	$FILE_OUTPUT.snps.filt.vcf \
#		> $FILE_OUTPUT.snps.2av.vcf
cp $FILE_OUTPUT.snps.filt.vcf $FILE_OUTPUT.snps.2av.vcf
...
#sed -i 's/Otherinfo/Quality,Filter,Genotype,DepthPerAllele,GenotypeQuality,PhredLikelyhood,Info/' \
...
sed -i 's/Otherinfo/Quality,Filter,Genotype,Format,Values/' \
       $FILE_OUTPUT.snps.av.exome_summary.csv
sed -i 's/Otherinfo/Quality,Filter,Genotype,Format,Values/' \
       $FILE_OUTPUT.snps.av.genome_summary.csv
...
# Fix AF entry in VCF file to actual AF
python $SCRIPT_FIXAF $FILE_OUTPUT.snps.filt.vcf $FILE_OUTPUT.snps.filt.RD.vcf
